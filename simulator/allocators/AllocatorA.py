...
class AllocatorA(Allocator):
    ...
    def allocate_for_agent_rec(self, agent: Agent, local_env: Environment,
        ...
        start_poi = agent.points_of_interest[0]
        for i in range(len(agent.points_of_interest) - 1):
            end_poi = agent.points_of_interest[i + 1]
            optimal_path = astar(start_poi.location,
                                 end_poi.location,
                                 agent,
                                 local_env,
