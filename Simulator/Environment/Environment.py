...
class Environment:
    ...
    def allocate_path(self, agent: Agent, path: List[TimeCoordinate]):
        if agent in self.agents:
            self.deallocate_agent(agent)
        else:
            self.agents.append(agent)

        agent.allocated_path = path
            ...
            for z in range(self.dimension.z):
                ...
                for y in range(self.dimension.y):
                    ...
                    for x in range(self.dimension.x):
                        ...
                        if field.is_allocated() and not self.is_blocked(field.coordinates):
